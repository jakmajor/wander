...
function Map({ trails = [{ lat: 0, long: 0 }], user, height = '700px' }) {
    const [liked, setLiked] = useState(false)

    const handleLikeClick = (trail_id) => {
        fetch("/hiker_trails", {
            method: "POST",
            headers: {
        ...
        <div className="marker-container">
            <div className={`marker-info ${`bg-${Math.floor(Math.random() * (3) + 1).toString()}`}`}>
                ...
                <div style={{ padding: '20px' }}>
                    ...
                    <img src={`${liked || (user.trails.filter(t => t.id === trail.id).length > 0) ? clickedLike : unclickedLike}`} onClick={() => handleLikeClick(trail.id)} />
                </div>
            </div>
    ...
    return (
        <> {trails.length > 0 &&
            <div className="google-map" style={{ height: height, width: '100%', borderRadius: '8px' }}>
                <GoogleMapReact
